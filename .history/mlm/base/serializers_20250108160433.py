...
class MLMUserSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        def assign_to_spot(node, new_user):
            if node.left is None and position == 'left':
                node.left = new_user
                node.save()
                print(f"Assigned user {new_user} to left of {node}")  # Debugging line
                return True
            elif node.right and position == 'right':
                node.right = new_user
                node.save()
                ...
                if position== 'left' and assign_to_spot(node.left, new_user):
                    return True
                elif position== 'rigth' and assign_to_spot(node.right, new_user):
                    return True
                else:
                    return False
