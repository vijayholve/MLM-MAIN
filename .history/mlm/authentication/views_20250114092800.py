...
class CustomTokenObtainPairView(TokenObtainPairView):
    def post(self, request, *args, **kwargs):
        try:
            ...
            res.set_cookie('csrftoken','access_token', access_token, httponly=True, secure=True, samesite='None', path='/')
            res.set_cookie('csrftoken'refresh_token', refresh_token, httponly=True, secure=True, samesite='None', path='/')
            return res
        except Exception as e:
            print(f"Error in CustomTokenObtainPairView: {e}")
            return Response({'success': False, 'error': 'Login failed. Please check your credentials.'}, status=400)
