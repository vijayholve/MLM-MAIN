...
const LoginUser = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setsubmitblock(true); // Block the submit button when the request is in progress
    ...
    try {
      ...
      setopenmessage(true);
      setsubmitblock(false); // Re-enable the submit button after success
    } catch (error) {
      console.log(error);
      ...
      setsubmitblock(false); // Re-enable the submit button after failure
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      <CssBaseline />
      <Card variant="outlined" sx={{ padding: 4, maxWidth: 500, margin: "0 auto" }}>
        <Typography
          component="h1"
          variant="h4"
          sx={{
        ...
        <Button type="submit" variant="contained" color="primary" disabled={submitblock}>
          Submit
        </Button>
