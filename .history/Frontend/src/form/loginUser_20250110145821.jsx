...
const LoginUser = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await axios.post('http://127.0.0.1:8000/api/login-page-2//', {
      username,
      password,
    });
    ...
    if (result.success) {
      ...
      setopenmessage(true); // Show success message
      nav("/home"); // Redirect to home page
    } else {
      setmessage({ success: "", failure: result.message });   
      setError({ general: result.message }); // Show error message
      setsubmitblock(false); // Re-enable the button
    }
  };
