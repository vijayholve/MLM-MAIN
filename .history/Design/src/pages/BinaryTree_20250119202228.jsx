...
const MLMTree = () => {
  ...
  const cleanTreeData = (data, currentLevel = 1, maxHeight = 3) => {
    if (!data || currentLevel > maxHeight) {
      return null;
    }
  
    const node = {
      name: data.name || "Empty Node",
      children: data.children || [],
    };
  
    // Ensure binary tree structure with placeholders
    ...
    node.children = node.children
      .slice(0, 2) // Limit to two children
      .map((child) => cleanTreeData(child, currentLevel + 1, maxHeight));
