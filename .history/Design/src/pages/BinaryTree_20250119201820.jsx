...
const MLMTree = () => {
  ...
  const cleanTreeData = (data, currentLevel = 1, maxHeight = 3) => {
    if (Array.isArray(data)) data = data[0];
  
    if (currentLevel > maxHeight) {
      return null; // Stop recursion if max height is exceeded
    }
    ...
    if (data.children) {
      data.children = data.children
        ...
        .map((child) => cleanTreeData(child, currentLevel + 1, maxHeight)) // Process children
        .filter((child) => child !== null); // Filter out null results
    } else {
      data.children = [];
    }
  
    // Root node: Ensure exactly 6 children
    if (currentLevel === 1 && data.children.length < 6) {
      while (data.children.length < 6) {
        data.children.push({
          data: { name: "Empty Node" },
          children: [],
        });
      }
    } else if (currentLevel === 1 && data.children.length > 6) {
      data.children = data.children.slice(0, 6); // Limit root node to 6 children
    }
  
    // Non-root nodes: Limit to 2 children
    if (currentLevel > 1 && data.children.length > 2) {
      data.children = data.children.slice(0, 2);
    }
