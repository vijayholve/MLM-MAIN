...
const MLMTree = () => {
  ...
  const cleanTreeData = (data) => {
    ...
    // Ensure the first (root) node has exactly 6 children
    if (data.isRoot && data.children.length < 6) {
    ...
    } else if (data.isRoot && data.children.length > 6) {
      data.children = data.children.slice(0, 6); // Limit to 6 children
    }
