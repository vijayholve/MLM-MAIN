...
const MLMTree = () => {
  ...
  const cleanTreeData = (data, currentLevel = 1, maxHeight = 3) => {
    ...
    if (currentLevel > maxHeight) {
      return null; // Stop recursion if the maximum height is exceeded
    }
    ...
    data.children = data.children
      ...
      .map((child) => cleanTreeData(child, currentLevel + 1, maxHeight)); // Process valid children

    // Fill missing children with "Empty Node"
    while (data.children.length < 2) {
      data.children.push({
    ...
    // Limit to exactly 2 children (binary tree structure)
