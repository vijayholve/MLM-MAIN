...
const SiteConfigEditor = () => {
  ...
  const handleFileChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: file, // Store the File object for file inputs
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(formData).forEach((key) => {
      if (formData[key] instanceof File || typeof formData[key] === "string") {
        formDataObj.append(key, formData[key]); // Include files and text fields
      }
    });
    try {
...
<form onSubmit={handleSubmit} encType="multipart/form-data">
  ...
  <Box sx={{ mb: 2 }}>
    ...
    {formData.homepage_banner && 
           <img
           src={`${baseURL}${formData.homepage_banner}`}
           alt="Navbar"
           style={{ width: "100px", height: "auto", marginBottom: "10px" }}
         />
         }
