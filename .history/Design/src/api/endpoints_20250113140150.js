...
export const logout = async () => {
  try {
    ...
    return {
      success: false,
      error: error.response?.data?.error || "Logout failed or server error.",
    };
  }
};
...
export const authenticated_user = async () => {
    try {
      ...
      return { success: true, data: response.data }; // Wrapped user data in an object
    } catch (error) {
      console.error("Authentication check failed:", error.response?.data || error.message);
      return {
        success: false,
        error: error.response?.data?.error || "Authentication check failed or server error.",
      };
    }
  };
